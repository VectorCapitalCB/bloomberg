 ...
 *
 * <p>Rules enforced on every call to {@link #populate}:
 * <ul>
...
public class SimulatorEngine {
    ...
    private static final double MAX_DRIFT_PCT    = 0.005;    // ±0.5 %
    private static final double MAX_SPREAD_BPS   = 0.0010;   // 10 bps
    ...
    public static void populate(BookSnapshot snapshot, SimulatorPriceRef ref) {
        ...
        // ---- half-spread: 1..5 bps of refPx (total ≤ 10 bps) ---------- //
        ...
        // ---- mid price drifts within band, keeping bid & ask inside ---- //
        ...
        // Guarantee bid < ask after rounding
        ...
        snapshot.getStatistic()
                ...
                .setLast(lastPx).setPreviusClose(prevClose)
                .setOpen(ref.getOpen() > 0 ? ref.getOpen() : refPx)
                .setHigh(highPx).setLow(lowPx).setClose(lastPx)
                .setVolume(volume).setVwap(vwap).setAmount(amount)
                .setTradeVolume(volume).setDelta(ref.getVariation());

        snapshot.getOhlcv()
                .setOpen(ref.getOpen() > 0 ? ref.getOpen() : refPx)
                .setHigh(highPx).setLow(lowPx).setClose(lastPx).setVolume(volume)
                .setSymbol(snapshot.getSymbol())
                .setSecurityExchange(snapshot.getSecurityExchangeMarketData())
    ...
    /**
     * Generates a standalone {@link MarketDataMessage.Trade} for global broadcast
     * (not tied to any individual subscription).
     *
    ...
    public static MarketDataMessage.Trade generateGlobalTrade(
        ...
        double px    = roundToTick(
                Math.max(refPx * 0.995, Math.min(refPx * 1.005, refPx + drift)), tick);
     ...
     * approximating BCS market conventions.
     */
